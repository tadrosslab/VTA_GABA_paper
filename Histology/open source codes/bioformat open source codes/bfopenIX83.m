function [IM, result] = bfopenIX83(id, zoomSeries, varargin)
%function [IM, result] = bfopenIX83(id, zoomSeries, varargin)
%similar to bfopen except limits numSeries to zoomSeries (defaults to ==1), also pulls out the images and concatenates them
%
%
...
if nargin<2 || isempty(zoomSeries)
    numSeries = 1; %r.getSeriesCount();  %IX83 specific hack
else
    numSeries = zoomSeries; %user-specified.
end
result = cell(length(numSeries), 2);

globalMetadata = r.getGlobalMetadata();

for ss=1:length(numSeries)
    s = numSeries(ss);
    ...
    for i = 1:numImages
        ...
        if bpp == 1
            colorMaps{ss, i} = r.get8BitLookupTable()';
        else
            colorMaps{ss, i} = r.get16BitLookupTable()';
        end

        warning_state = warning ('off');
        if ~isempty(colorMaps{ss, i})
            newMap = single(colorMaps{ss, i});
            newMap(newMap < 0) = newMap(newMap < 0) + bppMax;
            colorMaps{ss, i} = newMap / (bppMax - 1);
        end
    ...
    result{ss, 2} = seriesMetadata;
    result{ss, 3} = colorMaps;
    result{ss, 4} = r.getMetadataStore();
    %fprintf('\n');
end
r.close();
